...
help_message () {
	echo ""
	echo "Usage: metaWRAP read_qc [options] -1 reads_1.fastq.gz -2 reads_2.fastq.gz -o output_dir"
	echo "Note: the read files have to be named in the name_1.fastq.gz/name_2.fastq.gz convention."
	echo "Options:"
	echo ""
...
if [ "$trim" = true ]; then
	...
	mv ${out}/${sample}_1_val_1.fq.gz ${out}/trimmed_1.fastq.gz
	mv ${out}/${sample}_2_val_2.fq.gz ${out}/trimmed_2.fastq.gz
	if [[ ! -s ${out}/trimmed_1.fastq.gz ]]; then error "Something went wrong with trimming the reads. Exiting."; fi
	comm "Trimmed reads saved to: ${out}/trimmed_1.fastq.gz and ${out}/trimmed_2.fastq.gz"
	
	reads_1=${out}/trimmed_1.fastq.gz
	reads_2=${out}/trimmed_2.fastq.gz
	
	rm ${out}/${sample}_1_trimmed.fq.gz ${out}/${sample}_2_trimmed.fq.gz
fi
...
if [ "$bmtagger" = true ]; then
	...
	${SOFT}/skip_human_reads.py ${out}/${sample}.bmtagger.list $reads_1 > ${out}/${sample}_1.clean.fastq.gz
	${SOFT}/skip_human_reads.py ${out}/${sample}.bmtagger.list $reads_2 > ${out}/${sample}_2.clean.fastq.gz
	
	comm "Now sorting out found human reads and putting them into a new file... for science..."
	${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_1 > ${out}/host_reads_1.fastq.gz
	${SOFT}/select_human_reads.py ${out}/${sample}.bmtagger.list $reads_2 > ${out}/host_reads_2.fastq.gz
	if [[ ! -s ${out}/${sample}_1.clean.fastq.gz ]]; then error "Something went wrong with removing contaminant reads with bmtagger. Exiting."; fi

	rm -r ${out}/bmtagger_tmp
	rm ${out}/${sample}.bmtagger.list
	reads_1=${out}/${sample}_1.clean.fastq.gz
	reads_2=${out}/${sample}_2.clean.fastq.gz

	rm ${out}/trimmed_1.fastq.gz ${out}/trimmed_2.fastq.gz
fi	

mv $reads_1 ${out}/final_pure_reads_1.fastq.gz
mv $reads_2 ${out}/final_pure_reads_2.fastq.gz
comm "Contamination-free and trimmed reads are stored in: ${out}/final_pure_reads_1.fastq.gz and ${out}/final_pure_reads_2.fastq.gz"
...
if [ "$post_qc_report" = true ]; then
	...
	fastqc -t $threads -o ${out}/post-QC_report -f fastq ${out}/final_pure_reads_1.fastq.gz and ${out}/final_pure_reads_2.fastq.gz
